...
def grafico_barras_crecimiento(df, ax=None):
    ...
    bars = ax.barh(range(len(df)), valores, color=colores, edgecolor="white")
    for i, (v, bar) in enumerate(zip(valores, bars)):
        if v >= 0:
            ax.text(v + 0.5, bar.get_y() + bar.get_height()/2, f"{v:.1f}%",
                    va="center", fontsize=6, color="#333")
        else:
            ax.text(v - 0.5, bar.get_y() + bar.get_height()/2, f"{v:.1f}%",
                    va="center", ha="right", fontsize=6, color="#333")
...
def grafico_area_comparativa(df, ax=None, max_items=15):
    ...
    x = np.arange(len(top))
    ancho = 0.35
    v_2016 = top["area_total_2016"].values / 1e4
    v_2025 = top["area_total_2025"].values / 1e4
    b1 = ax.bar(x - ancho/2, v_2016, ancho, label="2015-2016", color="#c4a7cf")
    b2 = ax.bar(x + ancho/2, v_2025, ancho, label="2024-2025", color="#9b59b6")
    for bar, v in zip(b1, v_2016):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.05,
                f"{v:.1f}", ha="center", va="bottom", fontsize=6, color="#666")
    for bar, v in zip(b2, v_2025):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.05,
                f"{v:.1f}", ha="center", va="bottom", fontsize=6, color="#666")
    ax.set_xticks(x)
...
def leyenda_general_html():
    """HTML con la leyenda de todas las capas disponibles."""
    capas = [
        ("Límite Comunal", "#8B0000", "polygon"),
        ("Área Construida 2024-2025", "#FFAA00", "polygon"),
        ("Área Construida 2015-2016", "#FF4444", "polygon"),
        ("Red Hidrográfica", "#1565C0", "line"),
        ("Red Vial", "#E05000", "line"),
        ("Tasa de crecimiento", None, "graduated"),
    ]
    items = ""
    for nombre, color, tipo in capas:
        if tipo == "graduated":
            items += '<div style="margin:4px 0;font-size:11px;color:#444;"><i>Ver leyenda específica abajo</i></div>'
        elif tipo == "line":
            items += f"""
            <div style="display:flex;align-items:center;margin:3px 0;">
              <div style="background:{color};width:18px;height:3px;margin-right:7px;flex-shrink:0;"></div>
              <span style="font-size:11px;color:#222;">{nombre}</span>
            </div>"""
        else:
            items += f"""
            <div style="display:flex;align-items:center;margin:3px 0;">
              <div style="background:{color};width:18px;height:14px;
                          border:1px solid #555;margin-right:7px;border-radius:2px;flex-shrink:0;"></div>
              <span style="font-size:11px;color:#222;">{nombre}</span>
            </div>"""
    return f"""
    <div style="
      ...
      <b style="font-size:12px;color:#333;">Capas</b>
