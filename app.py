...
readme_path = DATA.parent / "README.md"
...
ARCHIVOS = {
    ...
    "Tasa de crecimiento": "tasa_crecimiento_localidades_futrono.geojson",
}
...
for nombre in capas_activas:
    ...
    elif "Tasa de crecimiento" in nombre:
        folium.GeoJson(
            data_json, name=nombre,
            style_function=lambda f: {
                "color": "#333333",
                "weight": 2,
                "fillColor": ac.COLORES_CLASIFICACION.get(f["properties"].get("grado_crec", ""), "#CCCCCC"),
                "fillOpacity": 0.7,
            },
            tooltip=folium.GeoJsonTooltip(fields=campos) if campos else None,
        ).add_to(m)

if "Tasa de crecimiento" in capas_activas:
    m.get_root().html.add_child(folium.Element(ac.leyenda_clasificacion_html()))

m.get_root().html.add_child(folium.Element(ac.leyenda_general_html()))
...
if "Tasa de crecimiento" not in capas:
