...
ARCHIVOS = {
    ...
    "Área Construida 2015-2016": "pac_2015_futrono.gpkg",
    "Diferencia Área Construida 2015-2025": "dac_2015-2025_1.gpkg",
...
def sanitizar_geodataframe(gdf):
    """Convierte columnas no serializables a tipos JSON-safe."""
    import numpy as np
    for col in gdf.columns:
        if col == gdf.geometry.name:
            continue
        if gdf[col].dtype in ("datetime64[ns]", "datetime64[us]", "<M8[ns]"):
            gdf[col] = gdf[col].astype(str)
        elif gdf[col].dtype in ("timedelta64[ns]", "<m8[ns]"):
            gdf[col] = gdf[col].astype(str)
    ...
    if ruta.exists():
        try:
            ...
            if gdf.crs is not None:
                gdf = gdf.to_crs(4326)
            capas[nombre] = sanitizar_geodataframe(gdf)
...
for nombre in capas_activas:
    ...
    if "Límite" in nombre:
        folium.GeoJson(
            gdf, name=nombre,
            style_function=lambda f, e=ESTILO_LIMITE: e,
        ...
        folium.GeoJson(
            gdf, name=nombre,
            style_function=lambda f, e=ESTILO_LAGOS: e,
            tooltip=folium.GeoJsonTooltip(fields=list(gdf.columns[:3])),
        ).add_to(m)
        ...
        folium.GeoJson(
            gdf, name=nombre,
            style_function=lambda f, e=ESTILO_PAC: e,
            tooltip=folium.GeoJsonTooltip(fields=list(gdf.columns[:3])),
        ).add_to(m)

    elif "Diferencia" in nombre:
        folium.GeoJson(
            gdf, name=nombre,
            style_function=lambda f, e=ESTILO_PAC: e,
            tooltip=folium.GeoJsonTooltip(fields=list(gdf.columns[:3])),
        ...
        folium.GeoJson(
            gdf, name=nombre,
            style_function=lambda f, e=ESTILO_RED_HIDRO: e,
            tooltip=folium.GeoJsonTooltip(fields=list(gdf.columns[:3])),
        ).add_to(m)
        ...
        folium.GeoJson(
            gdf, name=nombre,
            style_function=lambda f, e=ESTILO_RED_VIAL: e,
            tooltip=folium.GeoJsonTooltip(fields=list(gdf.columns[:3])),
        ).add_to(m)
