    ...
    if ruta.exists():
        try:
            ...
            if len(gdf) == 0:
                ...
                continue
            crs_actual = gdf.crs
            if crs_actual is None:
                gdf = gdf.set_crs("EPSG:32718")
                crs_actual = gdf.crs
            if crs_actual is not None and crs_actual.to_epsg() != 4326:
                gdf = gdf.to_crs(4326)
            capas[nombre] = {"gdf": gdf, "geojson": gdf_a_geojson(gdf)}
    ...
    if "Límite" in nombre:
        ...
        folium.GeoJson(
            ...
            style_function=lambda f: {
                ...
                "fillColor": "#FF4444", "fillOpacity": 0.7,
            },
            tooltip=folium.GeoJsonTooltip(fields=cols) if cols else None,
            show=True,
        ).add_to(m)
